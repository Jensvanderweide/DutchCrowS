...
def prob_next_token(matrix, token_ids, next_idx, lm):
    ...
    Args:
        matrix: Output matrix from model
        token_ids: Token IDs of the sentence
        next_idx: Index of the next token
        lm: Dictionary containing model components
        
    Returns:
        dict: Dictionary with log probability
    """
    model = lm["model"]
    ...
    # Use log_softmax layer to convert model scores for masked token to log-prob
    log_prob = log_softmax(next_token_scores)[target_word_id]

    return {'log_prob':log_prob}

def compare_pair(pair, lm): 
    """Compare a pair of sentences based on token probabilities
    
    Args:
        pair: Dictionary containing sentence pair
        lm: Dictionary containing model components
    ...
    # Determine which sentence is preferred (similar to first code)
    if sent1_log_probs > sent2_log_probs:
        preferred = "A"
    elif sent2_log_probs > sent1_log_probs:
        preferred = "B"
    else:
        preferred = "Equal"  # Keep track of equal cases
...
def evaluate(lm, data, sample_size=None, model_name=None):
    ...
    with tqdm(total=total) as pbar:
        for _, entry in eval_data.iterrows():
            ...
            # Handle direction and scoring similarly to first code
            if direction == 'stereo':
                total_stereo += 1
                if score['preferred'] == "A":  # If first sentence is preferred
                    stereo_score += 1
                    pair_score = 1
            else:  # direction == 'antistereo'
                total_antistereo += 1
                if score['preferred'] == "B":  # If second sentence is preferred
                    antistereo_score += 1
                    pair_score = 1
            
            # Count neutral cases separately
            ...
            # Append result to the list - match structure with first code
            results.append({
                'sent_more': sent_more,
                'sent_less': sent_less,
    ...
    df_score.to_csv('stereotype_evaluation_results.csv', index=False)  # Match filename with first code

    print('=' * 100)
    print(f'STEREOTYPE EVALUATION RESULTS - {model_name}')  # Match output formatting with first code
    print('=' * 100)
